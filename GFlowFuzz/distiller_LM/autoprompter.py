...
from GFlowFuzz.distiller_LM.utils import create_config, request_engine
...
class AutoPrompter:
    ...
    def __init__(
        ...
        self.folder = folder
        self.logger = logger
        self.wrap_prompt = wrap_prompt_func
        self.validate_prompt = validate_prompt_func
    ...
    def generate_prompt(
        ...
        hw_prompt: Optional[str] = None,
        hw: bool = False,
        no_input_prompt: bool = False,
        model: str = "gpt-4",
        num_samples: int = 3,
        max_tokens: int = 500
    ) -> str:
        ...
        Args:
            ...
            model: LLM model to use for auto-prompting
        ...
        if no_input_prompt:
            self.logger.logo("Without any input prompt ... ", level=LEVEL.INFO)
            best_prompt = (
                f"{self.prompt_components['separator']}\n{self.prompt_components['begin']}"
            )
        elif hw:
            self.logger.logo("Use handwritten prompt ... ", level=LEVEL.INFO)
            best_prompt = self.wrap_prompt(hw_prompt)
        else:
